...
def getColorThumbPath(red, green, blue):
    thumbPath = getThumbDir() + "\\" + hex((red << 16) +
                                           (green << 8) + blue) + ".png"
    return thumbPath
    ...
    def query(self, query):
        ...
        if len(params) == 3 or len(params) == 4:
            try:
                ...
                color = "#{color:0{padding}x}".format(color=color,
                                                      padding=len(params) * 2)
                params.reverse()
                icon = createColorThumb(int(params[2]), int(params[1]),
                                        int(params[0]))
